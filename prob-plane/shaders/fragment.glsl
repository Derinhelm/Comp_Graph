...
//float DistanceEvaluation(float3 v, )//!!!!!!!!!!!!!!!!!!!!!!!!!!! разный набор параметров для разных объектов
//{
   //return sdPlane(ray_pos, vec4(normalize(vec3(0.0f, 1.0f, 0.0f)), 1.0f));
//}

float3 EstimateNormal(vec3 n) //!!!!!!!!!!!!!!!!!!!!!!!!!!! разный набор параметров для разных объектов
{
    return n;
}
...
float RayIntersection(vec3 ray_pos, vec3 ray_dir){
    ...
    for (int i = 0; i < max_steps; i++) {
        float dist = sdPlane(ray_pos, vec4(normalize(vec3(0.0f, 1.0f, 0.0f)), 1.0f));
        if (abs(dist) < mindist) {
            return alldist; 
        } 
...
{	
  ...
  if (RayIntersection(ray_pos, ray_dir) ==  -1.0f) {
      ...
      if (length((light + RayIntersection(light, -lightDir) * (-lightDir)) - point_pos) > 0.1f) {
          ...
          vec3 norm = normalize(EstimateNormal(vec3(0.0f, 1.0f, 0.0f)));//костыль!!!!!!!!!!! нормаль
